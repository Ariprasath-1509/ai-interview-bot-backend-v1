...
public class DigestService {
    ...
    public void sendDailyDigest() {
        log.info("Sending comprehensive daily platform report...");
        try {
            ...
            log.info("Daily report sent to {} recipients", recipients.size());
        } catch (Exception e) {
    ...
    private String buildHtmlReport(Map<String, Object> reportData, Map<String, Object> pipelineStatus) {
        ...
        html.append("<!DOCTYPE html><html lang='en'><head>")
            ...
            .append("<title>Daily Platform Report</title>")
            .append("</head>")
            ...
            .append("<table width='100%' cellpadding='0' cellspacing='0' style='background:#f0f2f5;padding:30px 15px;'>")
            .append("<tr><td align='center'>");

        // ── WRAPPER ───────────────────────────────────────────────────────────
        html.append("<table width='640' cellpadding='0' cellspacing='0' style='background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);'>");

        // ── HEADER ────────────────────────────────────────────────────────────
        html.append("<tr><td style='background:linear-gradient(135deg,#1e3a5f 0%,#2d6a9f 100%);padding:32px 30px;text-align:center;'>")
            .append("<div style='display:inline-block;background:rgba(255,255,255,0.12);padding:8px 20px;border-radius:20px;margin-bottom:12px;'>")
            .append("<span style='color:#ffffff;font-size:13px;font-weight:600;letter-spacing:1px;text-transform:uppercase;'>Bench Readiness Platform</span>")
            .append("</div>")
        ...
        // ── ALERTS BANNER (only if there are issues) ──────────────────────────
        if (totalWithdrawn > 0 || pendingReviewCount > 0) {
            ...
            if (totalWithdrawn > 0) {
                html.append("<td style='color:#856404;font-size:13px;font-weight:600;'>")
                    .append("⚠️ ").append(totalWithdrawn).append(" integrity violation").append(totalWithdrawn > 1 ? "s" : "").append(" today")
                    .append("</td>");
            }
            if (pendingReviewCount > 0) {
                html.append("<td style='color:#856404;font-size:13px;font-weight:600;text-align:right;'>")
                    .append("🕐 ").append(pendingReviewCount).append(" interview").append(pendingReviewCount > 1 ? "s" : "").append(" pending review &gt;24h")
                    .append("</td>");
            }
        ...
        html.append(summaryCard("Interviews Today", String.valueOf(totalToday), "#2d6a9f", "#e8f0fe"));
        html.append(summaryCard("Ready", String.valueOf(readyCount), "#059669", "#d1fae5"));
        html.append(summaryCard("Success Rate", String.format("%.0f%%", successRate), successRate >= 70 ? "#059669" : successRate >= 40 ? "#d97706" : "#dc2626", successRate >= 70 ? "#d1fae5" : successRate >= 40 ? "#fef3c7" : "#fee2e2"));
        html.append(summaryCard("Violations", String.valueOf(totalWithdrawn), totalWithdrawn > 0 ? "#dc2626" : "#6b7280", totalWithdrawn > 0 ? "#fee2e2" : "#f3f4f6"));
        ...
        if (!interviewList.isEmpty()) {
            html.append("<tr><td style='padding:10px 30px 20px;'>")
                .append(sectionTitle("📋 Today's Interviews"))
                ...
                .append("<th style='padding:10px 12px;text-align:center;color:#374151;font-weight:600;border-bottom:1px solid #e5e7eb;'>Verdict</th>")
                .append("</tr>");
            ...
            for (int i = 0; i < interviewList.size(); i++) {
                ...
                String verdict = interview.get("finalVerdict") != null
                        ? String.valueOf(interview.get("finalVerdict"))
                        : interview.get("proposedVerdict") != null
                            ? String.valueOf(interview.get("proposedVerdict")) + "*"
                            : "Pending";

                html.append("<tr style='background:").append(bg).append(";'>")
                    .append("<td style='padding:9px 12px;color:#111827;border-bottom:1px solid #f3f4f6;'>")
                        .append(esc(String.valueOf(interview.getOrDefault("candidateName", "Unknown"))))
                        .append("<br><span style='font-size:11px;color:#9ca3af;'>")
                        .append(esc(String.valueOf(interview.getOrDefault("candidateEmail", ""))))
                        .append("</span></td>")
                    .append("<td style='padding:9px 12px;color:#374151;border-bottom:1px solid #f3f4f6;'>")
                        .append(esc(String.valueOf(interview.getOrDefault("jdTitle", "-")))).append("</td>")
                    .append("<td style='padding:9px 12px;text-align:center;border-bottom:1px solid #f3f4f6;'>")
                        .append(modeBadge(String.valueOf(interview.getOrDefault("mode", "-")))).append("</td>")
                    .append("<td style='padding:9px 12px;text-align:center;border-bottom:1px solid #f3f4f6;'>")
                        .append(statusBadge(status)).append("</td>")
                    .append("<td style='padding:9px 12px;text-align:center;border-bottom:1px solid #f3f4f6;'>")
                        .append(verdictBadge(verdict)).append("</td>")
                    .append("</tr>");
            }
            html.append("</table>")
                .append("<p style='margin:6px 0 0;font-size:11px;color:#9ca3af;'>* proposed verdict, pending admin sign-off</p>")
                .append("</td></tr>");
        }
        ...
        if (!violationDetails.isEmpty()) {
            html.append("<tr><td style='padding:10px 30px 20px;'>")
                .append(sectionTitle("🚨 Integrity Violations"))
                ...
                .append("<th style='padding:10px 12px;text-align:left;color:#991b1b;font-weight:600;border-bottom:1px solid #fca5a5;'>Candidate</th>")
                .append("<th style='padding:10px 12px;text-align:left;color:#991b1b;font-weight:600;border-bottom:1px solid #fca5a5;'>Reason</th>")
                .append("<th style='padding:10px 12px;text-align:left;color:#991b1b;font-weight:600;border-bottom:1px solid #fca5a5;'>Time</th>")
                .append("</tr>");

            for (Map<String, Object> v : violationDetails) {
                html.append("<tr>")
                    .append("<td style='padding:9px 12px;color:#111827;border-bottom:1px solid #fee2e2;'>")
                        .append(esc(String.valueOf(v.getOrDefault("candidateName", "Unknown"))))
                        .append("<br><span style='font-size:11px;color:#9ca3af;'>")
                        .append(esc(String.valueOf(v.getOrDefault("candidateEmail", "")))).append("</span></td>")
                    .append("<td style='padding:9px 12px;color:#dc2626;font-weight:600;border-bottom:1px solid #fee2e2;'>")
                        .append(esc(String.valueOf(v.getOrDefault("reason", "Unknown")))).append("</td>")
                    .append("<td style='padding:9px 12px;color:#6b7280;font-size:12px;border-bottom:1px solid #fee2e2;'>")
                        .append(formatTimestamp(String.valueOf(v.getOrDefault("timestamp", "")))).append("</td>")
                    .append("</tr>");
            }
        ...
        if (!pendingReviews.isEmpty()) {
            html.append("<tr><td style='padding:10px 30px 20px;'>")
                .append(sectionTitle("🕐 Pending Review (&gt;24 hours)"))
                ...
                .append("<tr style='background:#fef3c7;'>")
                .append("<th style='padding:10px 12px;text-align:left;color:#92400e;font-weight:600;border-bottom:1px solid #fcd34d;'>Candidate</th>")
                .append("<th style='padding:10px 12px;text-align:left;color:#92400e;font-weight:600;border-bottom:1px solid #fcd34d;'>Role</th>")
                .append("<th style='padding:10px 12px;text-align:center;color:#92400e;font-weight:600;border-bottom:1px solid #fcd34d;'>Waiting</th>")
                .append("</tr>");

            for (Map<String, Object> p : pendingReviews) {
                long hours = ((Number) p.getOrDefault("hoursWaiting", 0)).longValue();
                html.append("<tr>")
                    .append("<td style='padding:9px 12px;color:#111827;border-bottom:1px solid #fef3c7;'>")
                        .append(esc(String.valueOf(p.getOrDefault("candidateName", "Unknown")))).append("</td>")
                    .append("<td style='padding:9px 12px;color:#374151;border-bottom:1px solid #fef3c7;'>")
                        .append(esc(String.valueOf(p.getOrDefault("jdTitle", "-")))).append("</td>")
                    .append("<td style='padding:9px 12px;text-align:center;color:#d97706;font-weight:600;border-bottom:1px solid #fef3c7;'>")
                        .append(hours).append("h</td>")
                    .append("</tr>");
            }
            html.append("</table></td></tr>");
        }
        ...
        if (pipelineStatus != null) {
            html.append("<tr><td style='padding:10px 30px 20px;'>")
                .append(sectionTitle("👥 Candidate Pipeline"))
                .append("<table width='100%' cellpadding='0' cellspacing='0'><tr>");

            html.append(pipelineCard("RFD", rfd, "Ready for Deployment", "#059669", "#d1fae5"));
            html.append(pipelineCard("WFD", wfd, "Waiting for Deployment", "#2563eb", "#dbeafe"));
            html.append(pipelineCard("DOB", dob, "Deploy Observe on Bill", "#7c3aed", "#ede9fe"));
            html.append(pipelineCard("Deployed", deployed, "Currently Deployed", "#0891b2", "#cffafe"));

            html.append("</tr></table>");

            if (todayRegistrations > 0) {
                html.append("<p style='margin:10px 0 0;font-size:13px;color:#059669;font-weight:600;'>")
                    .append("✅ ").append(todayRegistrations).append(" new candidate").append(todayRegistrations > 1 ? "s" : "").append(" registered today")
                    .append("</p>");
            }
            html.append("<p style='margin:6px 0 0;font-size:12px;color:#9ca3af;'>Total candidates in system: ").append(totalCandidates).append("</p>")
                .append("</td></tr>");
        }

        // ── CTA BUTTON ────────────────────────────────────────────────────────
        html.append("<tr><td style='padding:20px 30px 30px;text-align:center;'>")
            .append("<a href='").append(dashboardUrl).append("' style='display:inline-block;background:linear-gradient(135deg,#1e3a5f 0%,#2d6a9f 100%);color:#ffffff;text-decoration:none;padding:14px 36px;border-radius:8px;font-weight:600;font-size:15px;'>")
            .append("View Full Dashboard →")
            .append("</a>")
            .append("</td></tr>");

        // ── FOOTER ────────────────────────────────────────────────────────────
        html.append("<tr><td style='background:#f9fafb;padding:20px 30px;text-align:center;border-top:1px solid #e5e7eb;'>")
            .append("<p style='margin:0;color:#6b7280;font-size:12px;'>Bench Readiness Platform · Automated Daily Report · ").append(date).append("</p>")
            .append("<p style='margin:4px 0 0;color:#9ca3af;font-size:11px;'>This report is sent every day at 7:00 PM IST to all admins.</p>")
            .append("</td></tr>");

        html.append("</table>"); // wrapper
        html.append("</td></tr></table>"); // outer
    ...
    private String pipelineCard(String label, int count, String subtitle, String color, String bg) {
        return "<td style='padding:0 6px 0 0;width:25%;'>" +
               "<div style='background:" + bg + ";border-radius:8px;padding:14px 12px;text-align:center;'>" +
               ...
               "<div style='font-size:10px;color:#6b7280;margin-top:2px;'>" + subtitle + "</div>" +
               "</div></td>";
    }
    ...
    private String statusRow(String label, int count, String color, boolean altBg) {
        ...
        return "<tr style='background:" + bg + ";'>" +
               "<td style='padding:10px 16px;color:#374151;font-size:13px;border-bottom:1px solid #f3f4f6;'>" + label + "</td>" +
               "<td style='padding:10px 16px;text-align:right;border-bottom:1px solid #f3f4f6;'>" +
               "<span style='background:" + color + ";color:#fff;font-size:12px;font-weight:700;padding:3px 10px;border-radius:12px;'>" + count + "</span>" +
               "</td></tr>";
    }
    ...
    private String statusBadge(String status) {
        String color = switch (status) {
            case "SIGNED_OFF" -> "#059669";
            case "REVIEW_PENDING" -> "#d97706";
            case "IN_PROGRESS" -> "#2563eb";
            case "COMPLETED" -> "#7c3aed";
            case "SCHEDULED" -> "#6b7280";
            default -> "#dc2626";
        };
        return "<span style='background:" + color + ";color:#fff;font-size:11px;font-weight:600;padding:2px 8px;border-radius:10px;white-space:nowrap;'>"
                + status.replace("_", " ") + "</span>";
    }

    private String verdictBadge(String verdict) {
        if (verdict == null || verdict.equals("Pending")) {
            return "<span style='color:#9ca3af;font-size:12px;'>Pending</span>";
        }
        String color = switch (verdict.replace("*", "")) {
            case "READY" -> "#059669";
            case "NEEDS_1_WEEK_PREP" -> "#d97706";
            case "NEEDS_RESKILLING" -> "#dc2626";
            case "MISMATCH_WITH_JD" -> "#7c3aed";
            case "WITHDRAWN" -> "#dc2626";
            default -> "#6b7280";
        };
